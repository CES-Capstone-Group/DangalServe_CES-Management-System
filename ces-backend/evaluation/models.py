...
# Response model: Tracks user responses to a specific evaluation form
class Response(models.Model):
    response_id = models.AutoField(primary_key=True)
    form = models.ForeignKey(EvaluationForm, on_delete=models.CASCADE, related_name="responses")
    user = models.ForeignKey('api.Account', on_delete=models.CASCADE, related_name="responses")
    submitted_at = models.DateTimeField(default=timezone.now)

    def __str__(self):
        return f"Response {self.response_id} - User: {self.user.username} - Form: {self.form.title}"


# Answer model: Stores individual answers for a user's response
class Answer(models.Model):
    answer_id = models.AutoField(primary_key=True)
    response = models.ForeignKey(Response, on_delete=models.CASCADE, related_name="answers")
    question = models.ForeignKey(Question, on_delete=models.CASCADE, related_name="answers")
    section_option = models.ForeignKey(
        RatingOpt,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="answers",  # For rating-type questions
    )
    question_option = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="answers",  # For multiple-choice questions
    )
